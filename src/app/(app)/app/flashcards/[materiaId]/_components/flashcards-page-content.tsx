...
import { FileText, MoveLeft, Play } from "lucide-react"
import Link from "next/link"
import { NewFlashcardDialog } from "./dialog-new-flashcard"

export function FlashcardsPageContent({ materiaId }: { materiaId: string }) {
    ...
    <>
      <div className="flex items-start mb-4 md:flex-row md:items-center justify-between md:mb-8">
        ...
        <div className="flex gap-4 flex-wrap">
          ...
          <NewFlashcardDialog materiaId={materiaId} onSuccess={() => setRefreshKey(k => k + 1)} />
        </div>
      </div>
      ...
      <FlashcardList materiaId={materiaId} refreshTrigger={refreshKey} />
    </>
  )
}
