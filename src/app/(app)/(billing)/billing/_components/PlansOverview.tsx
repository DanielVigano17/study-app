...
export default function PlansOverview({ plans, newUser, onSelectPlan }: PlansOverviewProps) {
  return newUser ? 
  <NewUserPlansOverview plans={plans} newUser={newUser} onSelectPlan={onSelectPlan} /> 
  : <ExistingUserPlansOverview plans={plans} newUser={newUser} onSelectPlan={onSelectPlan} />;
} 



function NewUserPlansOverview({ plans, onSelectPlan }: PlansOverviewProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-3xl font-bold mb-2">Teste Gratuitamente por 7 dias</h2>
        <p className="text-muted-foreground">
          Ative sua assinatura escolhendo o plano ideal para você e teste gratuitamente por 7 dias
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {plans.map((plan) => {
          ...
          return (
            <Card 
              key={plan.id} 
              className={`relative flex flex-col justify-between transition-all duration-200 hover:shadow-lg`}
            >
              {plan.id === 'pro' && (
...
function ExistingUserPlansOverview({ plans, onSelectPlan }: PlansOverviewProps) {
  const [loadingPlanId, setLoadingPlanId] = useState<string | null>(null);
  const [isAnnual, setIsAnnual] = useState<boolean>(false);
